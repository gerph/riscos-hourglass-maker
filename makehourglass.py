...
def make_objasm(rows, rowdata, deltas, images_rowindexes, filename, bitness):
    ...
    if bitness == 32:
        ...
        lines.append('        MACRO')
        lines.append('$label  SWI     $name')
        lines.append('$label')
        lines.append('        MOV     x10, #$name :AND: &FFFF')
        lines.append('        ADD     x10, x10, #$name :AND: &FFFF0000')
        lines.append('        SVC     #0')
        lines.append('        MEND')
        lines.append('')
    ...
    if bitness == 32:
        ...
        lines.append("          MOV     x0, #hg_workspacesize")
        lines.append("          RET")
    lines.append("")
    ...
    if bitness == 32:
        ...
        lines.append("          STR     w0, hg_framenum")
        lines.append("          STR     w0, hg_leds")
        lines.append("          MOV     w0, #-1")
        lines.append("          STR     w0, hg_percentagenow")
        lines.append("          MOV     w0, #100")
        lines.append("          STR     w0, hg_percentage")
        ...
        lines.append("          LSL     w2, w1, #16                 ; move lower hard of word into the upper half")
        lines.append("          ORR     w0, w0, w2                  ; assign the low half word of pointer data")
    ...
    if bitness == 32:
        ...
        lines.append("          MOV     x0, #0")
        lines.append("          STR     w0, hg_framenum")
        lines.append("          MOV     x0, #1")
        lines.append("          MOV     x1, #25                     ; read pointer colour 1")
        lines.append("          SWI     XOS_ReadPalette")
        lines.append("          BVC     %FT10")
        lines.append("          LDR     w2, =&81397900              ; purple if it wasn't set")
        lines.append("10")
        lines.append("          STR     w2, hg_oldcolours + (4 * 0)")
        lines.append("          MOV     x0, #2")
        lines.append("          MOV     x1, #25                     ; read pointer colour 2")
        lines.append("          SWI     XOS_ReadPalette")
        lines.append("          BVC     %FT20")
        lines.append("          LDR     w2, =&66FFFF00              ; yellow if it wasn't set")
        lines.append("20")
        lines.append("          STR     w2, hg_oldcolours + (4 * 1)")
        lines.append("          MOV     x0, #3")
        lines.append("          MOV     x1, #25                     ; read pointer colour 3")
        lines.append("          SWI     XOS_ReadPalette")
        lines.append("          BVC     %FT30")
        lines.append("          LDR     w2, =&00000000              ; black if it wasn't set")
        lines.append("30")
    ...
    if bitness == 32:
        ...
        lines.append("          MOV     x1, #127                        ; invalid pointer number to read the pointer")
        lines.append("          MOV     x0, #106                        ; select pointer")
        lines.append("          SWI     XOS_Byte")
        lines.append("          BVC     %FT10")
        lines.append("          MOV     x1, #0                          ; if there was an error, turn off")
        lines.append("10")
        lines.append("          STR     w1, hg_oldpointer")
    ...
    for colour_number in range(len(shape.palette) - 1):
        if bitness == 32:
            ...
            lines.append("; colour {}".format(colour_number + 1))
            lines.append("          ADR     x2, hg_oldcolours + (4 * {}) + 1".format(colour_number))
    ...
    if bitness == 32:
        ...
        lines.append("          MOV     x1, x4                          ; re-select the old pointer number")
        lines.append("          MOV     x0, #106                        ; select pointer")
        lines.append("          SWI     XOS_Byte")
    ...
    if bitness == 32:
        ...
        lines.append("          MOV     x29, sp")
        lines.append("          MOV     x12, x0")
        lines.append("          LDR     w0, hg_framenum")
        lines.append("          ADR     x1, frame_deltas")
        lines.append("          LDR     w0, [x1, x0, LSL #2]            ; offset within deltas for this frame")
        lines.append("          ADR     x1, deltas")
        lines.append("          ADD     x1, x1, x0")

    if not direct_rowdata:
        if bitness == 32:
            lines.append("          ADRL    r2, rowdata")
        else:
            lines.append("          ADRL    x2, rowdata")
    if bitness == 32:
        lines.append("          ADRL    r5, hg_currentdata")
    else:
        lines.append("          ADRL    x5, hg_currentdata")
    ...
    if direct_rowdata:
        if bitness == 32:
            ...
            lines.append("          LDRSW   x3, [x1], #4                    ; r3 = currentdata offset")
            lines.append("          CMP     x3, #-1                         ; end of the rows")
            lines.append("          BEQ     rowend")

            lines.append("      [ wordsperrow = 1")
            lines.append("          LDR     w0, [x1], #4                    ; read a word")
            lines.append("          STR     w0, [x3, x5]                    ; store into the currentdata")
            lines.append("      ]")
            lines.append("      [ wordsperrow = 2")
            lines.append("          LDP     w0, w2, [x1], #8                ; read two word from rowdata")
            lines.append("          ADD     x3, x3, x5                      ; work out the line offset")
            lines.append("          STR     w0, [x3]                        ; store into the currentdata")
            lines.append("          STR     w2, [x3, #4]                    ; store into the currentdata")
            lines.append("      ]")
            lines.append("      [ wordsperrow = 3")
            lines.append("          LDP     w0, w2, [x1], #8                ; read two words from rowdata")
            lines.append("          LDR     w4, [x1]                        ; read final word from rowdata")
            lines.append("          ADD     x3, x3, x5                      ; work out the line offset")
            lines.append("          STP     w0, w2, [x3], #8                ; store two words to currentdata")
            lines.append("          STR     w4, [x3], #4                    ; store final word to currentdata")
            lines.append("      ]")
            lines.append("      [ wordsperrow = 4")
            lines.append("          LDP     w0, w2, [x1], #8                ; read two words from rowdata")
            lines.append("          LDP     w4, r5, [x1]                    ; read final words from rowdata")
            lines.append("          ADD     r3, r3, r5                      ; work out the line offset")
            lines.append("          STP     w0, w2, [x3], #8                ; store two words to currentdata")
            lines.append("          STP     w4, w5, [x3], #4                    ; store final words to currentdata")
            lines.append("      ]")
    else:
        if bitness == 32:
            ...
            lines.append("          LDP     w3, w4, [x1], #8                ; r3 = currentdata offset, r4 = row data offset")
            lines.append("          SXTW    x3, w3")
            lines.append("          CMP     x3, #-1                         ; end of the rows")
            lines.append("          BEQ     rowend")

            lines.append("          LDR     w0, [x4, w2]!                   ; read a word from rowdata")
            lines.append("          STR     w0, [x3, w5]!                   ; store into the currentdata")

            lines.append("      [ wordsperrow > 1")
            lines.append("          LDR     w0, [x4, #4]!                   ; read a word from rowdata")
            lines.append("          STR     w0, [x3, #4]!                   ; store into the currentdata")
            lines.append("      ]")
            lines.append("      [ wordsperrow > 2")
            lines.append("          LDR     w0, [x4, #4]!                   ; read a word from rowdata")
            lines.append("          STR     w0, [x3, #4]!                   ; store into the currentdata")
    ...
    if percentage_height:
        if bitness == 32:
            ...
            lines.append("          LDR     w4, hg_percentage")
            lines.append("          LDR     w0, hg_percentagenow")
            lines.append("          CMP     w0, w4")
            lines.append("          BEQ     percentage_up_to_date")
            lines.append("")
            lines.append("          CMP     w4, #100")
            lines.append("          MOV     w0, #height")
            lines.append("          BEQ     %FT20")
            lines.append("          MOV     w0, #height + percentage_height")
            lines.append("20")  # The above would be better as a conditional load
            lines.append("          STRB    w0, hg_word + 3                 ; update pointer height for percentage block")
            lines.append("          STR     w4, hg_percentagenow")
            lines.append("          BEQ     percentage_up_to_date")
            lines.append("")
            lines.append("          ADR     x1, percentage_offsets")
            lines.append("          LDR     w0, [x1, x4, LSL #2]            ; offset within percentage data")
            lines.append("          ADR     x1, percentage_data")
            lines.append("          ADD     x1, x1, x0")

            lines.append("          ADD     w3, w5, #wordsperrow * 4 * height ; move to the end of the frame data")

            lines.append("          LDR     w0, [x1, #4]!                   ; read a word from rowdata")
            lines.append("          STR     w0, [x3, #4]!                   ; store into the currentdata")

            lines.append("      [ percentage_worddata_size > 4")
            lines.append("          LDR     w0, [x1, #4]!                   ; read a word from rowdata")
            lines.append("          STR     w0, [x3, #4]!                   ; store into the currentdata")
            lines.append("      ]")
            lines.append("      [ percentage_worddata_size > 8")
            lines.append("          LDR     w0, [x1, #4]!                   ; read a word from rowdata")
            lines.append("          STR     w0, [x3, #4]!                   ; store into the currentdata")
            lines.append("      ]")
            lines.append("      [ percentage_worddata_size > 12")
            lines.append("          LDR     w0, [x1, #4]!                   ; read a word from rowdata")
            lines.append("          STR     w0, [x3, #4]!                   ; store into the currentdata")
            lines.append("      ]")
            lines.append("      [ percentage_worddata_size > 16")
            lines.append("          LDR     w0, [x1, #4]!                   ; read a word from rowdata")
            lines.append("          STR     w0, [x3, #4]!                   ; store into the currentdata")
            lines.append("      ]")
            lines.append("      [ percentage_worddata_size > 20")
            lines.append("          LDR     w0, [x1, #4]!                   ; read a word from rowdata")
            lines.append("          STR     w0, [x3, #4]!                   ; store into the currentdata")
            lines.append("      ]")
            lines.append("      [ percentage_worddata_size > 24")
            lines.append("          LDR     w0, [x1, #4]!                   ; read a word from rowdata")
            lines.append("          STR     w0, [x3, #4]!                   ; store into the currentdata")
            lines.append("      ]")
    ...
    if bitness == 32:
        ...
        lines.append("          MOV     x0, #21                         ; Set Pointer parameters")
        lines.append("          ADR     x1, hg_word")
        lines.append("          SWI     XOS_Word")

        # It'd be better if we changed the pointer shape when we got the next VSync, but I'm being lazy.
        lines.append("")
        lines.append("          LDRB    w1, hg_word + 1                 ; get the hourglass pointer number we will use")
        lines.append("          MOV     w5, #7")
        lines.append("          SUB     w4, w5, w1                      ; toggles 3 to 4")
        lines.append("          STRB    w4, hg_word + 1                 ; toggles the pointer number we use next time")
        lines.append("          MOV     x0, #106                        ; select pointer")
        lines.append("          SWI     XOS_Byte")

        lines.append("")
        lines.append("          LDR     w0, hg_framenum")
        lines.append("          ADD     w0, w0, #1")
        lines.append("          CMP     w0, #nframes")
        lines.append("          BNE     %FT10")
        lines.append("          MOV     w0, #0                          ; reset counter; we've cycled")
        lines.append("10")
        lines.append("          STR     w0, hg_framenum")
    ...
    if bitness == 32:
        ...
        # FIXME: I'm not switching mode here
        lines.append("          BL      hourglass_frame")
        # FIXME: Not restoring mode
    ...
    if bitness == 32:
        ...
        lines.append("          MOV     x12, x0")
        lines.append("          STR     x1, hg_percentage")
...
make_objasm(rows, rowdata, deltas, images_rowindexes, filename='asm', bitness=32)
make_objasm(rows, rowdata, deltas, images_rowindexes, filename='asm64', bitness=64)
